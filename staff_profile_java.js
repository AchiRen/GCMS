...
function saveField(first_name) {
    ...
    xhr.open('POST', 'staff_update.php', true);
...
function editField(group_id) {
    document.getElementById(group_id).style.display = 'none';
    document.getElementById('edit-' + group_id).style.display = 'inline-block';
}

function saveField(group_id) {
    var newValue = document.getElementById('edit-' + group_id).value;

    var xhr = new XMLHttpRequest();
    xhr.onreadystatechange = function() {
        if (xhr.readyState === 4) {
            if (xhr.status === 200) {
                document.getElementById(group_id).textContent = newValue;
                document.getElementById('edit-' + group_id).style.display = 'none';
                document.getElementById(group_id).style.display = 'inline-block';
            } else {
                console.error('Error updating field: ' + group_id);
            }
        }
    };
